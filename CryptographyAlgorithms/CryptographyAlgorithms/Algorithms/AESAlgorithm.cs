...
{
	public class AES_EncryptionAlgorithm
	{
	...
	{0xe7,0x19,0x4f,0xa8,0x9a,0x83},{0xe5,0x1a,0x46,0xa3,0x97,0x8d}
		...
		private T[] SubArray<T>(T[] data, int index, int length = -1)
		{
			if (length == -1)
		...
		private void AddRoundKey(byte[,] state, uint[] w)
		{
		...
		private uint KE_ROTWORD(uint x)
		{
			return (((x) << 8) | ((x) >> 24));
		}

		public uint[] KeyExpansion(byte[] key, uint[] w, int keysize)
		{
		...
		public byte[] Encrypt(byte[] input, byte[] output, uint[] key, int keysize)
		{
	...
	public class AES_DecryptionAlgorithm
    {
		private static byte[,] aes_sbox = {
	{0x63,0x7C,0x77,0x7B,0xF2,0x6B,0x6F,0xC5,0x30,0x01,0x67,0x2B,0xFE,0xD7,0xAB,0x76},
	{0xCA,0x82,0xC9,0x7D,0xFA,0x59,0x47,0xF0,0xAD,0xD4,0xA2,0xAF,0x9C,0xA4,0x72,0xC0},
	...
	{0x8C,0xA1,0x89,0x0D,0xBF,0xE6,0x42,0x68,0x41,0x99,0x2D,0x0F,0xB0,0x54,0xBB,0x16}
		...
		private static byte[,] aes_invsbox = {
	...
	{0xe7,0x19,0x4f,0xa8,0x9a,0x83},{0xe5,0x1a,0x46,0xa3,0x97,0x8d}
		...
		public byte[] Decrypt(byte[] input, byte[] output, uint[] key, int keysize)
		{
			byte[,] state = new byte[4, 4];
			...
			if (keysize > 128)
			{
				if (keysize > 192)
				{
					...
					InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 52)); InvMixColumns(state);
					InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 48)); InvMixColumns(state);
				}
				else
				{
					AddRoundKey(state, SubArray(key, 48));
				}
				InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 44)); InvMixColumns(state);
				InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 40)); InvMixColumns(state);
			}
			else
			{
				AddRoundKey(state, SubArray(key, 40));
			}
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 36)); InvMixColumns(state);
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 32)); InvMixColumns(state);
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 28)); InvMixColumns(state);
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 24)); InvMixColumns(state);
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 20)); InvMixColumns(state);
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 16)); InvMixColumns(state);
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 12)); InvMixColumns(state);
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 8)); InvMixColumns(state);
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 4)); InvMixColumns(state);
			InvShiftRows(state); InvSubBytes(state); AddRoundKey(state, SubArray(key, 0));
