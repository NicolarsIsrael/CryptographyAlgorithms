...
namespace CryptographyAlgorithms.Algorithms
{
	public class DES_Cryptography
	{
		...
		private uint BITNUM(byte[] a, int b, int c)
		{
			return (uint)(((a[b / 8] >> (7 - (b % 8))) & 0x01) << (c));
		}

		private byte BITNUMINTR(uint a, int b, int c)
		{
			return (byte)((((a) >> (31 - (b))) & 0x00000001) << (c));
		}

		private uint BITNUMINTL(uint a, int b, int c)
		{
			return ((((a) << (b)) & 0x80000000) >> (c));
		}

		private uint SBOXBIT(byte a)
		{
	...
	2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11
		...
		public void KeySchedule(byte[] key, byte[,] schedule, uint mode)
		{
			uint i, j, toGen, C, D;
		...
		private void IP(uint[] state, byte[] input)
		{
		...
		private byte[] InvIP(uint[] state, byte[] input)
		{
		...
		private uint F(uint state, byte[] key)
		{
		...
		public byte[] Crypt(byte[] input, byte[] output, byte[][] key)
		{
